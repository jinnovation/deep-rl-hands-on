...
def iterate_batches(
    ...
    env_gen: Iterator[gym.Env] = iter(lambda: random.choice(envs), None)
...
@click.command()
@click.option("--cuda", default=False, type=bool, help="Enable CUDA")
def main(cuda):
    ...
    for batch_v in iterate_batches(envs):
        # Generate random vector and pass it to the generator network
        gen_input_v = torch.FloatTensor(BATCH_SIZE, LATENT_VECTOR_SIZE, 1, 1)
        gen_input_v.normal_(0, 1).to(device)
        batch_v = batch_v.to(device)
        gen_output_v = net_gener(gen_input_v)

        # Train discriminator by applying twice:
        #   - To the true data samples from batch; then
